...
const UserManagement = () => {
    ...
    if (user.role === "learner") {
      ...
      try {
        ...
        if (resp.status === "success") {
          ...
          setEnrolledCourseCodes(
            courses.filter((c) => c.enrolled).map((c) => c.code),
          );
          // codes đang in-batch = đã thanh toán + đã vào lớp
          setLockedCourseCodes(
            courses.filter((c) => c.inBatch).map((c) => c.code),
          );
        }
      } catch (e) {
  ...
  const columns = [
    ...
    //           <span
    //             key={code}
    //             className="px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 text-xs font-medium border border-indigo-100"
    //           >
    //             {code}
    //           </span>
    //         ))}
    //         {(record.enrolledCourseCodes || []).length === 0 && (
              ...
              {loadingEnrolled ? (
                ...
                <form onSubmit={handleEnrolmentSubmit} className="space-y-6">
                  <div className="space-y-6 max-h-[60vh] overflow-y-auto px-1">
                    {/* Nhóm Xe Máy (A*) */}
                    {(() => {
                      const xeMayCourses = enrolledCoursesData.filter((c) =>
                        /^A[12]$/.test(c.code),
                      );
                      if (xeMayCourses.length === 0) return null;

                      // Kiểm tra xem có bất kỳ course nào trong nhóm này bị locked không
                      const anyLocked = xeMayCourses.some((c) =>
                        lockedCourseCodes.includes(c.code),
                      );
                      const selected = xeMayCourses.filter((c) =>
                        ...
                        <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4">
                          ...
                          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                            {xeMayCourses.map((course) => {
                              const isCourseLocked = lockedCourseCodes.includes(
                                course.code,
                              );
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex flex-col">
                                    <span
                                      className={`text-sm font-bold ${isDisabled ? "text-slate-500" : "text-slate-900"}`}
                                    >
                                      Hạng {course.code}
                                    </span>
                    ...
                    {(() => {
                      ...
                      const anyLocked = oToCourses.some((c) =>
                        lockedCourseCodes.includes(c.code),
                      );

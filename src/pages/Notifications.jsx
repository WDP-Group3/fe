    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-sm backdrop-blur">
        ...
        <div className="mb-6 flex items-center gap-4">
          <span className="text-sm font-medium text-slate-700">Lọc theo loại:</span>
          <select
            value={filterType}
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600">
                      {TYPE_TITLES_Notification.find(t => t.value === item.type)?.label || item.type}
                    </span>
                  </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            <h3 className="mb-4 text-lg font-bold text-slate-900">{formData.id ? 'Sửa thông báo' : 'Tạo thông báo mới'}</h3>
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">Loại thông báo</label>
                <select
                  required
                  value={formData.type}
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">Thời gian hết hạn (ngày)</label>
                <input
                  type="number"
                  min="1"
                  required
                  ...
                  className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
                />
                <p className="mt-1 text-xs text-slate-500">Thông báo sẽ tự động xóa sau số ngày này.</p>
              </div>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mb-4 flex flex-wrap gap-2 text-xs">
              ...
              <span className="rounded-full bg-indigo-50 px-3 py-1 font-medium text-indigo-700">
                {TYPE_TITLES_Notification.find(t => t.value === selectedNotification.type)?.label}
              </span>
            </div>
